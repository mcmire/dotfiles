...
"
" <kbd>%</kbd> is one of my favorite keys -- it lets you bounce between the
" start and end of `if` statements, loops, blocks in Ruby, etc. This mapping is
" provided by the [`matchit`][matchit] plugin, which we have to enable:
"
...
" <kbd>C</kbd> is the same thing as saying <kbd>c</kbd><kbd>$</kbd>;
" <kbd>D</kbd> is the same as <kbd>d</kbd><kbd>$</kbd>. So why not do the same
" for <kbd>y</kbd><kbd>$</kbd>?

nnoremap Y y$

" For some reason a while back, I found it useful to unmap <kbd>K</kbd>. I
" suppose I may find this useful still, even if I don't know it.

noremap K <Esc>

" In the [search settings](search.vim.md), we configure <kbd>/</kbd> to
" highlight matches. It's common to want to clear that highlight, so
...
" I use tabs a lot. <kbd>,</kbd><kbd>t</kbd><kbd>c</kbd> gives me a way to close
" a tab quickly without having to close all the buffers inside it:

nmap <Leader>tc :tabc<CR>

" I map Caps Lock to Escape. I realize this is a pretty unusual setup (although
" I really can't imagine why -- it's great!), so for Those People who are used
" to jamming <kbd>j</kbd><kbd>j</kbd>, <kbd>j</kbd><kbd>k</kbd>, or
