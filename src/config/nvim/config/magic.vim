...
" Finally, we bring in a heavy-hitter. As capable as Vim is, it lacks one
" feature that modern editors have wisely introduced. Say you're editing a file
" in Vim, but for some reason, you use another program to make changes to that
" file. What happens? Usually Vim does nothing. That doesn't work very well,
" because that means it's completely possible for you to make more changes
" inside of Vim that overwrite the first set of changes. Wouldn't it be great if
" Vim detected outside changes and automatically reloaded the file?
"
