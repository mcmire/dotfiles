...
"
" Let's start with soft wrapping. We do not turn on soft wrapping by default,
" but we do provide a way to turn it on with <kbd>,wi</kbd>:
...
" lines by using the usual <kbd>j</kbd> and <kbd>k</kbd> (normally, we would
" have to use <kbd>g</kbd><kbd>j</kbd> and <kbd>g</kbd><kbd>k</kbd>):
...
"   new lines using <kbd>o</kbd> or <kbd>O</kbd>, Vim will ensure that the
"   characters that start the comment (`#`, `//`, `"`, etc.) are present at the
"   beginning of the newly added line.
...
"   it manually with <kbd>Q</kbd> -- but it works well for most cases.
